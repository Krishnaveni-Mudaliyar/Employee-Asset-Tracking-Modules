codeunit 50101 "Asset Posting Mgt."
{
    TableNo = "Asset Assignment Header";
    ...
    procedure PostAssetAssignment(var pRecHeader: Record "Asset Assignment Header")
    var
    ...
    begin
        ...
        if lRecSetup."Require Approval" then
            if pRecHeader."Approval Status" <> pRecHeader."Approval Status"::Approved then
                Error('Assignment %1 must be approved before posting. Approval status: %2.', pRecHeader."No.", pRecHeader."Approval Status");
        ...
        lCodTelemetry.LogAssetAssigned('MULTI', pRecHeader."Employee No.", pRecHeader."No.");
    end;

    procedure SendForApproval(var pRecHeader: Record "Asset Assignment Header")
    begin
    ...
    local procedure InsertPostedHeader(var pRecHeader: Record "Asset Assignment Header")
    var
        lRecPostedHeader: Record "Posted Assignment Header";
    begin
    ...
    local procedure PostLines(var pRecHeader: Record "Asset Assignment Header")
    var
        lRecLine: Record "Asset Assignment Line";
    ...
    local procedure DeleteOriginalDocument(var pRecHeader: Record "Asset Assignment Header")
    var
        lRecLine: Record "Asset Assignment Line";
    begin
        pRecHeader.Status := pRecHeader.Status::Open;
        pRecHeader."Approval Status" := pRecHeader."Approval Status"::" ";
        pRecHeader.Modify(false);
    ...
    local procedure GetPostedHeader(pCodNo: Code[20]): Record "Posted Assignment Header"
    var
        lRecPostedHeader: Record "Posted Assignment Header";
    begin
