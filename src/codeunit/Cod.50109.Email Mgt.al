codeunit 50109 "Email Mgt."
{
    ...
    begin
        ...
        lMsg.Create(lSetup."Admin Email Address",
            StrSubstNo('%1 Overdue Return(s) - Action Required - %2',
            pCount,
            Format(Today)),
            lBody, true);
        lEmail.Send(lMsg);
    ...
    begin
        ...
        lAsset.SetFilter(
            ...
            '>=%1&<=%2',
            Today,
            Today + lSetup."Warranty Alert Days");
        ...
        lBody := StrSubstNo(
            '<h2>%1 Asset Warranty Expiry/Expiries in Next %2 Days</h2>',
            lCount,
            lSetup."Warranty Alert Days");
        ...
        if lAsset.FindSet() then
            repeat
                lBody += StrSubstNo(
                    ...
                    lAsset."No.",
                    lAsset.Description,
                    Format(lAsset."Warranty Expiry Date"),
                    lAsset.Status);
            until lAsset.Next() = 0;
        lBody += '</table>';

        lMsg.Create(lSetup."Admin Email Address",
            StrSubstNo('%1 Asset Warranty Expiries in %2 Days',
            lCount,
            lSetup."Warranty Alert Days"),
            lBody, true);
        lEmail.Send(lMsg);
    ...
    begin
        ...
        lBody += StrSubstNo(
            '<tr><td><b>Assignment No.</b></td><td>%1</td></tr>',
            pHdr."No.");

        lBody += StrSubstNo(
            '<tr><td><b>Employee</b></td><td>%1 (%2)</td></tr>',
            pHdr."Employee Name",
            pHdr."Employee No.");

        lBody += StrSubstNo(
            '<tr><td><b>Department</b></td><td>%1</td></tr>',
            pHdr.Department);

        lBody += StrSubstNo(
            '<tr><td><b>Assignment Date</b></td><td>%1</td></tr>',
            Format(pHdr."Assignment Date"));

        lBody += StrSubstNo(
            '<tr><td><b>Expected Return</b></td><td>%1</td></tr>',
            Format(pHdr."Expected Return Date"));
    ...
    begin
        ...
        lBody := StrSubstNo(
            '<h2>Assignment %1 - %2</h2><p>Dear %3,</p>', 
            pHdr."No.", 
            lResult, pHdr."Employee Name");
    ...
    begin
        ...
        lBody := '<h2>Approval Required - Asset Assignment</h2>';
        lBody += StrSubstNo(
            '<p>Assignment <b>%1</b> submitted by %2 requires your approval.</p>',
            pHdr."No.", 
            pHdr."Approval Requested By");

        lBody += StrSubstNo(
            '<p>Employee: %1 | Department: %2 | Date: %3</p>',
                        pHdr."Employee Name", 
                        pHdr.Department, 
                        Format(pHdr."Assignment Date"));
    ...
    begin
        ...
        lBody += StrSubstNo(
            '<p>Assignment <b>%1</b> for employee %2 has been pending approval since %3.</p>',
                        pHdr."No.", 
                        pHdr."Employee Name", 
                        Format(pHdr."Approval Requested On"));

        lBody += '<p>Please action this immediately in Business Central.</p>';

        lMsg.Create(pManagerEmail,
            StrSubstNo('ESCALATED: Assignment %1 Awaiting Approval', 
            pHdr."No."),
            lBody, true);
        lEmail.Send(lMsg);
    end;
