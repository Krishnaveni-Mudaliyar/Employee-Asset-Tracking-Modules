codeunit 50104 "Asset Notification"
{
    ...
    begin
        ...
        lRecPostedHeader.SetRange(
            "Transaction Type",
            lRecPostedHeader."Transaction Type"::Assignment);

        lRecPostedHeader.SetFilter(
        ...
        if lIntCount = 1 then begin
            ...
            ('1 asset assignment is overdue for return. Employee: %1, Due: %2.',
            lRecPostedHeader."Employee No.",
            lRecPostedHeader."Expected Return Date");
        end else
            lNotification.Message := StrSubstNo
            ('%1 asset assignments are overdue for return. Review the Overdue Asset Return report.',
            lIntCount);

        lNotification.Scope := NotificationScope::LocalScope;
        lNotification.AddAction(
            'View Overdue Report',
    ...
    begin
        ...
        if lIntCount = 1 then begin
            lRecAsset.FindFirst();
            lNotification.Message := StrSubstNo(
                '1 asset warranty expires within 30 days. Asset: %1, Expiry: %2.',
                lRecAsset."No.",
                lRecAsset."Warranty Expiry Date");
        end else
            lNotification.Message := StrSubstNo(
                '%1 asset warranties expire within 30 days. Review your asset register.',
                lIntCount);
