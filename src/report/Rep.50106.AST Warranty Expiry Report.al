report 50106 "AST Warranty Expiry Report"
{
        ...
        dataitem(Asset; "AST Company Asset")
        {
            ...
            begin
                if "Warranty Expiry Date" = 0D then begin
                    DaysRem := 0; WStatus := 'No Warranty';
                end else begin
                    DaysRem := "Warranty Expiry Date" - Today;
                    if DaysRem < 0 then WStatus := 'EXPIRED'
                    else if DaysRem <= 7  then WStatus := 'CRITICAL – Expires this week'
                    else if DaysRem <= 30 then WStatus := 'Expiring Soon'
                    else WStatus := 'Active';
                end;
            end;
        }
    }
    var DaysRem: Integer; WStatus: Text[40];
}
